    ...
    function updateStyle(b: boolean){
        if(b!=parent.value){
            ...
            if(b){//was changed to parent
                document.getElementById('parentButton')?.classList.add('bg-stone-100');
                document.getElementById('parentButton')?.classList.remove('bg-bone-100');
                document.getElementById('childButton')?.classList.add('bg-bone-100');
                document.getElementById('childButton')?.classList.remove('bg-stone-100');
            }
            else{//was changed to child
                createH.value = false;
                document.getElementById('parentButton')?.classList.add('bg-bone-100');
                document.getElementById('parentButton')?.classList.remove('bg-stone-100');
                document.getElementById('childButton')?.classList.add('bg-stone-100');
                document.getElementById('childButton')?.classList.remove('bg-bone-100');
            }
        }
    }
        ...
        <div class="shadow-md bg-bone-200 border-bone-300 rounded-xl p-5 my-16 border-2 w-4/5 overflow-auto">
            ...
            <form class="text-sm sm:text-base">
                <div class="flex flex-col mb-4 w-1000 justify-center">
                    ...
                    <input v-model="name" placeholder="First name" class="bg-bone-50 rounded-md px-2"/>
                    <p class="font-museoModerno mb-2 mt-4">Last name: </p>
                    <input v-model="lName" placeholder="Last name" class="bg-bone-50 rounded-md px-2"/>
                    <div class = "flex flex-row mt-4 mb-2">
                        <p class="font-museoModerno mr-1">Birthday  </p>
                        <p class="font-sans">(YYYY/MM/DD): </p>
                    </div>
                    <div class="flex flex-row">
                        <input v-model="year" placeholder="1999" class="bg-bone-50 w-20 rounded-md px-2"/>
                        <input v-model="month" placeholder="10" class="bg-bone-50 w-10 rounded-md mx-2 px-2"/>
                        <input v-model="day" placeholder="29" class="bg-bone-50 w-10 rounded-md px-2"/>
                    </div>
                    
                </div>
                <p class="font-museoModerno">User selected: </p>
                <div class="w-1000 flex flex-row mt-2">
                    <button type="button" class="mr-6 rounded-md bg-bone-100 transition-all duration-100 hover:bg-stone-100 text-black px-6 py-2" 
                    @click="updateStyle(true)" id="parentButton"> 
                        Parent
                    </button>
                    <button type="button" class="rounded-md bg-stone-100 transition-all duration-100 hover:bg-stone-100 text-black px-6 py-2"
                ...
                <div v-if="!createH" class="w-1000 flex flex-col">
                    ...
                    <input v-model="houseHold" placeholder="House ID" class="bg-bone-50 rounded-md px-2" />
                
                    <div v-if="parent" class="flex justify-center items-center">
                        <button type="button" class="rounded-md bg-bone-100 transition-all duration-100 hover:bg-stone-100 text-black px-6 py-2 mt-4"
                        @click="createH=true">
                ...
                <div v-if="createH" class="w-1000 flex flex-col">
                    ...
                    <input v-model="houseName" placeholder="Household name" class="bg-bone-50 rounded-md px-2" />
                ...
                <div class="flex flex-col w-1000 justify-center items-center">
                    <v-btn type="submit" class="rounded-md w-1/3 bg-bone-100 transition-all duration-100 hover:bg-stone-100 text-black px-6 py-2 mt-4" 
                    @click.prevent="submitForm"> 
                        Submit 
                    </v-btn> 
                    <p class="text-sm text-bone-300">{{ formFeedback }}</p>
                </div>
            </form>
        </div>
    </div>
